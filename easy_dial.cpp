...
void fml(node * n, int div, int level, double &res,bool condon)const{
    if(!=NULL){
        res+=(n.frequencia/div)*level;
        fml(n->cen,div,level+1,res,true);
        if(condon){
            fml(n->cen,div,level+1,res,false);
            fml(n->cen,div,level+1,res,false);
        }else{
            fml(n->cen,div,level,res,false);
            fml(n->cen,div,level,res,false);
        }
    }
...
void easy_dial::buscahavueltomalditoesinmatable(node* it,vector<string>& s)const{
    if(it!=NULL){
        ...
        if(it!=NULL)s.push_back(it->cell.nom());
    }
};
